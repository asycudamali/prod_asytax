...
class MyCollableItemTaxationRules implements  Callable<Boolean>{
	...
	public MyCollableItemTaxationRules(HTReplicationRunner runner,PascalResourceFile rs){
		 this.runner = runner;
		 this.rs = rs;
	}
	...
	public Boolean call()  {
		...
		Vector v;
		boolean done;
		rs.setItemPos("GenTaxRules");
		int count = rs.getPascalInt();
		// DebugOutput.print("---> Item taxation rules (" + --count + ")");
		...
		for (int j = 0; j < count; j++) {
			...
			DebugOutput.print("Rule name ItemTaxation= " + name);
			int pri = 0;
			byte[] b = null;
