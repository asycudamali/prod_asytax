...
class MyCollableGlobalTaxationRules implements  Callable<Boolean>{
	...
	public MyCollableGlobalTaxationRules(HTReplicationRunner runner,PascalResourceFile rs){
		 this.runner = runner;
		 this.rs = rs;
	}
	...
	public Boolean call()  {
		...
		Vector v;
		boolean done;
		rs.setItemPos("GloTaxRules");
		int count = rs.getPascalInt();
		// DebugOutput.print("---> Global taxation rules (" + --count + ")");
		...
		for (int j = 0; j < count; j++) {
			...
			DebugOutput.print("Rule name GloTaxRules= " + name);
			int pri = 0;
			byte[] b = null;
